...
std::string decodeEncode(const std::string& input) {
  std::string decoded;
  CryptoPP::StringSource(input, true, new Converter_t(new CryptoPP::StringSink(decoded)));
  return decoded;
}
...
template <typename T>
concept SavableKey = CryptoMaterial<T> && requires(T key, CryptoPP::ByteQueue queue) { key.Save(queue); };

template <typename T>
concept LoadableKey = CryptoMaterial<T> && requires(T key, CryptoPP::ByteQueue queue) { key.Load(queue); };
...
std::optional<bool> checkFileSignatureFromManifest(const QString& fileToCheck, const QString& manifestPath,
                                                   const QString& verifierPath);
...
bool checkFile(const QString& inFilename, const CryptoPP::RSASSA_PKCS1v15_SHA_Verifier& verifier,
               const QString& signatureFile);
// check a signed file from the target file, a verifier and a signature
bool checkFileFromSign(const QString& inFilename, const CryptoPP::RSASSA_PKCS1v15_SHA_Verifier& verifier,
                       const QString& signatureq);
...
bool saveKeys(const Private_t& prvKey, const Public_t& pubKey, const std::string& prvKeyPath,
...
inline bool saveStringToFile(const std::string& content, const QString& fileName) {
...
inline std::string readFile(const QString& filename) {
  QFile f{filename};
...
inline std::tuple<CryptoPP::RSASSA_PKCS1v15_SHA_Signer, CryptoPP::RSASSA_PKCS1v15_SHA_Verifier> getKeyPair() {
...
inline void load(const QString& filename, CryptoPP::BufferedTransformation& bt) {
  QFile f{filename};
...
inline auto loadPrivateKey(const QString& filename, bool enableCheck = true) {
  return u_baseLoadKey<CryptoPP::RSASSA_PKCS1v15_SHA_Signer, CryptoPP::RSA::PrivateKey>(filename, enableCheck);
}
inline auto loadPublicKey(const QString& filename, bool enableCheck = true) {
...
std::string signFile(const std::string& filename, const Signer_t& signer) {
  ...
  try {
    CryptoPP::FileSource(
        filename.c_str(), true, new CryptoPP::SignerFilter{prng, signer, new CryptoPP::StringSink{signature}});
    signature = decodeEncode<CryptoPP::HexEncoder>(signature);
    qDebug() << "Signature:" << QString::fromStdString(signature);
  } catch (const std::exception& e) {
    qCritical() << "Following exception occured when trying to sign file:" << QString::fromStdString(filename) << ":"
                << e.what();
  } catch (...) {
...
inline bool checkFileFromSign(const QString& inFilename, const CryptoPP::RSASSA_PKCS1v15_SHA_Verifier& verifier,
                              std::string signature) {
...
inline bool checkFile(const QString& inFilename, const CryptoPP::RSASSA_PKCS1v15_SHA_Verifier& verifier,
                      const QString& signatureFile) {
  ...
  qInfo() << "Read signature file from <" << signatureFile << ">";
  qInfo() << "Read signature (size: " << signature.size() << ") <" << signature << ">";
...
inline std::optional<CryptoPP::RSASSA_PKCS1v15_SHA_Signer> loadSigner(const QString& inPath) {
  try {
...
inline std::optional<CryptoPP::RSASSA_PKCS1v15_SHA_Verifier> loadVerifier(const QString& inPath) {
  try {
    QFileInfo fInfo{inPath};
    if (inPath.isEmpty() || !fInfo.isReadable() || !fInfo.isFile()) {
      qCritical() << "Please enter a valid input file (not empty, in a readable dir)";
...
inline std::optional<bool> checkFileSignature(const QString& fileToCheck, const QString& publicKeyPath,
                                              const QString& fileExpectedSignature) {
  if (!QFileInfo::exists(publicKeyPath)) {
    qCritical() << "Verifier (public) key file does not exists:" << publicKeyPath;
    return {};
  }
  auto verifKeyOpt{signing::loadVerifier(publicKeyPath)};
  if (!verifKeyOpt) {
    qCritical() << "Aborting installation as we could not load the specified verifier (public) key file: <"
                << publicKeyPath << ">";
    return {};
  }
  const auto& verifKey{verifKeyOpt.value()};
  QFileInfo fInfo{fileToCheck};
  auto safeToInstall{signing::checkFileFromSign(fInfo.absoluteFilePath(), verifKey, fileExpectedSignature)};
  return safeToInstall;
}

inline std::optional<bool> checkFileSignatureFromManifest(const QString& fileToCheck, const QString& manifestPath,
  ...
  return checkFileSignature(fileToCheck, publicKeyPath, manifest.packageSignature);
}
