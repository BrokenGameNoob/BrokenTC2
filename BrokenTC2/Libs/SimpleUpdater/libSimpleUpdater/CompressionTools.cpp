...
int32_t getRecursiveFileCount(const QString& dir, const QString& workingDir) {
  ...
  if (QFileInfo{realPath}.isFile()) return 1;
...
bool saveBA(const QByteArray& ba, const QString& path) {
  ...
  qDebug() << "Saving to <" << path << "> (" << ba.size() << " bytes)";
...
QString Compressor::compress(const QString& outPath, QString workingDir) {
  ...
  if (!success) {
    qCritical() << "Could not open output file: " << outPath;
    return QString{};
  }
  QDataStream stream{&buff};
  ...
  for (const auto& p : m_customPaths) {
    ...
    qDebug().nospace() << "\t\tAdding to archive: <" << pathToSend << ">";
  ...
  qDebug() << "Compressed BA size: <" << ba.size() << "> bytes";

  if (saveBA(ba, outPath)) return outPath;
  return {};
}
...
bool Compressor::uncompress(const QString& path, const QString& outFolderPath) {
  ...
  if (!success) {
    qCritical() << "Can't open uncompressed archive file: <" << path << ">";
    return false;
  }
  QDataStream fileStream{&fileBuff};

  qDebug() << "Opening archive file: <" << path << "> -> <" << fileBuff.size() << ">";
  ...
  for (qint64 i{}; i < fileCount; ++i) {
    ...
    if (!success) {
      qCritical() << "Can't open compressed element in archive file: <" << path << ">";
      return false;
    }
    ...
    if (!qFInfo.absoluteDir().exists()) qFInfo.absoluteDir().mkpath(".");
...
bool Compressor::compressToStream(const QString& path, QDataStream& stream) {
  ...
  if (!inf.exists()) {
    qCritical() << "File does not exists: <" << inf.absoluteFilePath() << ">";
  } else
    qCritical() << "Unknown input type: <" << inf.absoluteFilePath() << ">";
  return false;
}
...
bool Compressor::compressFolder(const QString& root, const QString& path, QDataStream& stream) {
  ...
  while (it.hasNext()) {
    it.next();
    qDebug().nospace() << "\t\tCompressed file <" << it.filePath() << "> to <"
                       << fs::getFilePathFromRoot(root, it.filePath()) << ">";
...
bool Compressor::compressFile(const QString& path, QDataStream& stream) {
  qDebug().nospace() << "Compressed file <" << path << "> to <" << QFileInfo{path}.fileName() << ">";
