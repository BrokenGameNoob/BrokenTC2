...
DownloadManager::DownloadManager(QStringList urlList, QString outputPath, bool showProgress, QWidget* parent)
    : QObject{parent},
      m_progress{((showProgress) ? new QProgressDialog(QObject::tr("Download progress"), QObject::tr("Abort download"),
                                                       0, urlList.size() * 100, parent)
                                 : nullptr)},
...
void DownloadManager::startDownload(const QString& file) {
  auto tmpDlRequest{new DownloadRequest(file, GetOutputFile(file), this, false)};

  m_progressList[tmpDlRequest] = 0;
  m_rqList.emplace_back(tmpDlRequest);

  DownloadRequest::connect(
      tmpDlRequest, &DownloadRequest::error, this, [this, tmpDlRequest]() { slot_onDownloadError(tmpDlRequest); });
  ...
  DownloadRequest::connect(
      tmpDlRequest, &DownloadRequest::progress, this, [this, tmpDlRequest](int64_t curVal, int64_t size) {
...
void DownloadManager::slot_onDownloadSucceeded(DownloadRequest* rq) {
  qInfo() << "Successful download of: " << rq->url();
  m_outputFiles[rq->url()] = GetOutputFile(rq->url());
...
void DownloadManager::slot_onDownloadError(updt::net::DownloadRequest* rq) {
  if (m_failedDl.find(rq) != m_failedDl.end())  // Download already failed once...
    return;
  qCritical() << "Failed download of: " << rq->url();
