...
class UpdateHandler : public QWidget {
 ...
 public:
  ...
  struct InfoBoxMsgType {
    enum Type { kUnknown, kWarning, kOk, kCritical };
 ...
 public:
  explicit UpdateHandler(Version progRunningVersion, QString githubReleaseApiAddress, QString publicVerifierKeyFile,
                         bool searchAvailableOnCreation, QString postUpdateCmd,
                         bool showInstallPropositionOnNextOccasion, QWidget* parent);
  ~UpdateHandler();

  void showInstallPropositionOnNextOccasion() {
    m_showInstallPropositionOnNextOccasion = true;
  }
 ...
 public:
  static std::optional<qint64> startDetachedUpdateProcess(const QString& packagePath, const QString& verifierKeyPath,
                                                          const QString& updateManifest, const QString& postUpdateCmd);
  static std::optional<qint64> startProcess(const QString& executable, const QStringList& argsList = {},
                                            const QString& working_dir = QApplication::applicationDirPath());
 ...
 private:
  ...
  void setReadyToUpdate(bool state = true) {
    if (!state)
      resetState();
    else
  ...
  void onInstallerRetrieved(const QString& installer_path);

  void updateFromInstaller(const updt::Manifest& manifest);
  void updateFromPackage();
 ...
 private:
  ...
  const QRegularExpression m_kInstallerRegex{R"(setup.*x64\.exe)"};
