...
constexpr bool kDetailedLogs{false};

//-----------------------------------------------------------------------

void SDLEventThread::work(
    const QVector<GameController *> &controllerList,
  ...
  while (m_continue) {
    ...
    case SDL_JOYAXISMOTION: {
      ...
      auto dictKey{(AXIS_BUTTON_OFFSET + axis) * 1000 +
                   (e.jaxis.value < 0 ? 1 : 2) * 100 + deviceId()};

      if (!joyMove.contains(dictKey))
        joyMove[dictKey] = curPos;

      if (joyMove.at(dictKey) == false && curPos) {
        if constexpr (kDetailedLogs) {
          qInfo() << utils::HighResMs() << ": JOY AXIS (DOWN): " << deviceId()
                  << " : "
                  << (AXIS_BUTTON_OFFSET + axis * 10 +
                      (e.jaxis.value < 0 ? 1 : 2));
        }
        emit buttonDown(deviceId(), AXIS_BUTTON_OFFSET + axis * 10 +
                                        (e.jaxis.value < 0 ? 1 : 2));
        joyMove[dictKey] = true;
        lambdaSleep(15);
      } else if (joyMove.at(dictKey) == true && !curPos) {
        emit buttonUp(deviceId(), AXIS_BUTTON_OFFSET + axis * 10 +
                                      (e.jaxis.value < 0 ? 1 : 2));
    ...
    case SDL_JOYBUTTONDOWN:
      ...
      if constexpr (kDetailedLogs) {
        qInfo() << utils::HighResMs() << ": JOY Button (DOWN): " << deviceId()
                << " : " << e.jbutton.button;
      }
    ...
    case SDL_JOYHATMOTION: {
      ...
      auto checkPos{[&](const auto &hatVal) {
        ...
        if (curPos & hatVal) {
          if constexpr (kDetailedLogs) {
            qInfo() << utils::HighResMs()
                    << ": JOY Motion (DOWN): " << deviceId() << " : "
                    << HAT_BUTTON_OFFSET + hatVal;
          }
    ...
    case SDL_JOYDEVICEADDED:
      qInfo() << "New controller plugged in with id:" << e.jdevice.which
              << "emitting signal: newControllerPluggedIn(e.jdevice.which)";
      emit newControllerPluggedIn(e.jdevice.which);
      break;
    case SDL_JOYDEVICEREMOVED:
      qInfo() << "Controller unplugged with id:" << e.jdevice.which
              << "emitting signal: controllerUnplugged(e.jdevice.which)";
...
SDLEventHandler::SDLEventHandler() : QObject(), m_controllerList() {
  if (!initSDL(SDL_INIT_JOYSTICK)) {
    ...
    throw std::runtime_error(__CURRENT_PLACE_std_ +
                             " : Cannot initialize SDL2_Joystick");
  }
  ...
  connect(worker, &SDLEventThread::testSignal,
          [&]() { qDebug() << "J'ai eu un test !"; });

  connect(worker, &SDLEventThread::newControllerPluggedIn, this,
          [&](int id) { emit gameControllerAdded(id); });
  connect(worker, &SDLEventThread::controllerUnplugged, this,
          [&](int id) { emit gameControllerRemoved(id); });

  connect(worker, &SDLEventThread::buttonDown, this,
          &SDLEventHandler::onButtonDown);
  connect(worker, &SDLEventThread::buttonUp, this,
          &SDLEventHandler::onButtonUp);
...
void SDLEventHandler::registerController(GameController *g) {
  ...
  connect(g, &GameController::unregisterFromEventHandler, instance(),
          &SDLEventHandler::unregisterController);
...
void SDLEventHandler::unregisterController(GameController *g) {
  qInfo() << __PRETTY_FUNCTION__
          << "Unregistering controller with id:" << g->id();
  ...
  if (gIndex != -1)
    ...
    qWarning() << "Willing to unregister not known controller with id:"
               << g->id();
  }
}
