...
void sendKeyboardEvent(int code, bool pressed, int codeAlias) {
  ...
  std::ignore = codeAlias;
  if (pressed) {
    keyPress(MapVirtualKey(code, MAPVK_VK_TO_VSC));
  } else {
    keyRelease(MapVirtualKey(code, MAPVK_VK_TO_VSC));
  }
}

void delayedKeyboardEvent(int code, bool pressed, int delay) {
  QTimer::singleShot(delay, [=]() { windows::sendKeyboardEvent(code, pressed); });
}
