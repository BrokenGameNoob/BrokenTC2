...
void setKey(int keyCode, QLabel *lblDisp,
            tc::ProfileSettings::Key &settingsKeyToChange) {
  ...
  settingsKeyToChange =
      keyCode; // if key == -1, it will never be matched = Unbind
}

inline void setButton(int button, QLabel *lblDisp,
                      tc::ProfileSettings::Key &settingsBtnToChange) {
  ...
  settingsBtnToChange =
      button; // if button == -1, it will never be matched = Unbind
}
...
bool reg_startOnStartup(bool enableAutoStart) {
  ...
  HKEY hKey{};
  auto lResult{
      RegOpenKeyEx(HKEY_CURRENT_USER, runPath, 0, KEY_READ | KEY_WRITE, &hKey)};

  if (lResult != ERROR_SUCCESS) {
    if (lResult == ERROR_FILE_NOT_FOUND) {
      qCritical() << __CURRENT_PLACE__
                  << ": Registry key not found : " << cRunPath;
    } else {
      qCritical() << __CURRENT_PLACE__
                  << ": Error occured when opening registry key : " << cRunPath;
    }
    return false;
  }
  ...
  if (lResult != ERROR_SUCCESS) {
    if (lResult == ERROR_FILE_NOT_FOUND) {
      ...
      qCritical() << __CURRENT_PLACE__
                  << ": Error occured when writing registry key : " << cRunPath;
...
void SetLabelHasConflict(QLabel *lbl, const QString &conflict_with) {
  ...
  lbl->setToolTip(
      QObject::tr("This key is in conflict with <%1>").arg(conflict_with));
}
...
MainWindow::MainWindow(bool hideOnStartup, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow),
      ...
      m_updateHandler{new updt::UpdateHandler(
          consts::CURRENT_VERSION, consts::PROJECT_GITHUB_RELEASE,
          consts::PUBLIC_VERIFIER_KEY_FILE, true, consts::POST_UPDATE_CMD, true,
          this)},
      m_gearDisplay{new Widget_gearDisplay()}, m_softSettings{},
      c_appDataFolder{
          QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) +
          "/"},
      c_softSettingsFile{c_appDataFolder + "settings.conf"}, m_controller{},
      m_pixmapBg{} {
  ...
  auto lineList{
      this->findChildren<QFrame *>(QRegularExpression{"^line_[0-9]*$"})};
  for (auto &e : lineList) {
    e->setMidLineWidth(1);
  }
  ...
  connect(ui->toolB_help, &QToolButton::clicked, this, [&]() {
    ...
    if (!QDesktopServices::openUrl(QUrl{helpLink})) {
      QMessageBox::warning(
          this, tr("Can't help"),
          tr("Sorry, we can't open help. Go to this page :\n%0").arg(helpLink));
    }
  });
  connect(ui->pb_userManual, &QToolButton::clicked, this, [&]() {
    constexpr auto helpLink{
        "https://github.com/BrokenGameNoob/BrokenTC2#quick-guide"};
    qInfo() << __CURRENT_PLACE__ << " : open " << helpLink;
    if (!QDesktopServices::openUrl(QUrl{helpLink})) {
      QMessageBox::warning(
          this, tr("Can't help"),
  ...
  connect(windows::WindowsEventThread::ins(),
          &windows::WindowsEventThread::keyDown, this,
          &MainWindow::onKeyboardPressed);
#endif
  ...
  connect(ui->pb_selectKey_keyboardGearUp, &QPushButton::clicked, this, [&]() {
    auto key{GetKey()};
    setKey(key, ui->lbl_keyboardGearUp,
           m_gearHandler.settings().keyboardSeqGearUp);
    saveProfileSettings();
    UpdateConflicts();
  });
  connect(ui->pb_selectKey_keyboardGearDown, &QPushButton::clicked, this,
          [&]() {
            auto key{GetKey()};
            setKey(key, ui->lbl_keyboardGearDown,
                   m_gearHandler.settings().keyboardSeqGearDown);
            saveProfileSettings();
            UpdateConflicts();
          });
  ...
  connect(ui->pb_selectButton_cycleProfile, &QPushButton::clicked, this, [&]() {
    auto btn{Dialog_getGameControllerButton::getButton(&m_controller, this)};
    setButton(btn, ui->lbl_btn_cycleProfile,
              m_gearHandler.settings().cycleProfile);
    saveProfileSettings();
  });
  ...
  connect(ui->cb_use_seq_after_clutch, &QCheckBox::stateChanged, this,
          [&](int val) {
  ...
  ui->hs_joyAxisThreshold->setMaximum(
      std::numeric_limits<decltype(m_softSettings.joyAxisThreshold())>::max() -
      1); // int16_t
  ...
  connect(ui->cb_gearDisplayScreen, &QComboBox::currentIndexChanged, this,
          [&](int) {
            m_softSettings.displayGearScreen =
                ui->cb_gearDisplayScreen->currentText();

            m_gearDisplay->showOnScreen(
                ui->cb_gearDisplayScreen->currentData().toInt());
            saveSoftSettings();
          });
  ...
  connect(ui->cb_exitOnCloseEvent, &QComboBox::currentIndexChanged, this,
          [&](int valI) {
            m_softSettings.exitOnCloseEvent = bool(valI);
            saveSoftSettings();
          });
#ifdef Q_OS_WIN
  connect(ui->cb_launchOnStartup, &QComboBox::currentIndexChanged, this,
          [&](int valI) {
            reg_startOnStartup(bool(valI));
            saveSoftSettings();
          });
#endif
  connect(ui->tb_settings, &QTabWidget::currentChanged, this,
          [&](int index) { m_softSettings.openedTab = index; });

  connect(&m_gearHandler, &tc::GearHandler::gearChanged, m_gearDisplay,
          &Widget_gearDisplay::refreshGear);
  connect(&m_gearHandler, &tc::GearHandler::gearSwitchModeChanged,
          m_gearDisplay, &Widget_gearDisplay::onSwitchGearModeChanged);
  connect(&m_gearHandler, &tc::GearHandler::gearSwitchModeChanged, this,
          [&](auto) { saveProfileSettings(); });

  using qsdl::GameController;
  using qsdl::SDLEventHandler;
  connect(SDLEventHandler::instance(), &SDLEventHandler::gameControllerAdded,
          this, &MainWindow::onControllerPluggedIn);
  connect(SDLEventHandler::instance(), &SDLEventHandler::gameControllerRemoved,
          this, &MainWindow::onControllerUnplugged);

  connect(&m_controller, &GameController::buttonDown, this,
          &MainWindow::onControllerButtonPressed);

  //--------------------------------------------------------------------

  qDebug() << __CURRENT_PLACE__ << "   " << getCurrentProfileFilePath();
  if (!QFileInfo::exists(
          c_appDataFolder)) // if the appdata folder doesn't exist
  {
    if (!QDir::root().mkpath(c_appDataFolder)) {
      throw std::runtime_error{__CURRENT_PLACE_std_ +
                               " : Cannot create appData folder : <" +
  ...
  for (const auto &e : availableScreens) {
    ...
    const auto kDisplayName{screenCount[kTmpScreenName] > 1
                                ? QString{"%0_%1"}
                                      .arg(kTmpScreenName)
                                      .arg(screenCount[kTmpScreenName])
  ...
  if (!win::setCoreCountAffinity(m_softSettings.preferredCoreCount, false,
                                 true)) {
  ...
  m_wasUpdated =
      updt::acquireUpdated(toCallIfUpdated, lupdt::UPDATED_TAG_FILENAME, this);
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  if (event->spontaneous() &&
      !(m_softSettings.exitOnCloseEvent)) // if the window is closed by the top
                                          // right closing icon
  {
    this->hide();
    m_trayIcon.show();
...
void MainWindow::showEvent(QShowEvent *event) // when the window is shown
{
  ...
  if (m_softSettings.displayAboutOnStartup) { /*
    ...
    QMetaObject::invokeMethod(
        this, "on_action_about_triggered",
  ...
  if (m_wasUpdated) {
    QMetaObject::invokeMethod(
        this, [this]() { updt::showChangelog(this); },
...
bool MainWindow::setBackgroungImage(const QString &newPath) {
  ...
  ui->pb_changeBackground->setText(
      newPath == kDefaultBgPath || newPath.isEmpty() ? tr("Click to select")
                                                     : newPath);
...
void MainWindow::updateSoftSettings() {
  ...
  m_softSettings.currentDeviceName =
      ((ui->cb_selectDevice->currentText().isEmpty())
           ? m_softSettings.currentDeviceName
...
bool MainWindow::saveSoftSettings() {
  ...
  settings.insert("displayAboutOnStartup",
                  m_softSettings.displayAboutOnStartup);
...
bool MainWindow::loadSoftSettings() {
  ...
  m_softSettings.displayAboutOnStartup =
      settings.value("displayAboutOnStartup").toBool(true);
  ...
  m_softSettings.enableNotification =
      settings.value("enableNotification").toBool();
  m_softSettings.setLowPerfMode(settings.value("lowPerfMode").toBool());
  m_softSettings.displayGearScreen =
      settings.value("displayGearScreen").toString();
  m_softSettings.exitOnCloseEvent =
      settings.value("exitOnCloseEvent").toBool(false);
  m_softSettings.openedTab = settings.value("openedTab").toInt(def.openedTab);
  m_softSettings.preferredCoreCount =
      settings.value("preferredCoreCount").toInt(def.preferredCoreCount);
  auto bgHUDColorStr{settings.value("bgHUDColor").toString()};
  m_softSettings.setBgHUDColor(bgHUDColorStr.isEmpty()
                                   ? QColor{79, 79, 79, 120}
                                   : tc::stringToColor(bgHUDColorStr),
                               m_gearDisplay);
  m_softSettings.setJoyAxisThreshold(static_cast<int16_t>(
      settings.value("joyAxisThreshold")
          .toInt(std::numeric_limits<
                     decltype(m_softSettings.joyAxisThreshold())>::max() -
                 1)));
  m_softSettings.backgroundImagePath =
      settings.value("backgroundImagePath").toString();
...
bool MainWindow::loadProfileSettings() {
  try {
    m_gearHandler.settings() =
        tc::readProfileSettings(getCurrentProfileFilePath());
...
void MainWindow::refreshFromSettings() {
  ...
  auto deviceIndex{
      ui->cb_selectDevice->findText(m_softSettings.currentDeviceName)};
  if (deviceIndex >= 0) {
    ui->cb_selectDevice->setCurrentIndex(deviceIndex);
  }

  auto screenIndex{
      ui->cb_gearDisplayScreen->findText(m_softSettings.displayGearScreen)};
  if (screenIndex >= 0) {
  ...
  ui->cb_exitOnCloseEvent->setCurrentIndex(
      int(m_softSettings.exitOnCloseEvent));

  ui->tb_settings->setCurrentIndex(m_softSettings.openedTab);

  ui->hs_joyAxisThreshold->setValue(m_softSettings.joyAxisThreshold());

  ui->lbl_bgHUDColor->setStyleSheet(QString{"background-color:%0"}.arg(
      tc::colorToString(m_softSettings.bgHUDColor())));
...
void MainWindow::populateDevicesComboBox() {
  ...
  for (const auto &e : deviceList) {
    if (curDevice == e)
      newDeviceIndex = i;
    ui->cb_selectDevice->addItem(
        e, i); // store device id as data. Even though it should match cb index
    ++i;
  }
...
void MainWindow::refreshDisplayFromGearHandlerSettings() {
  ...
  lambdaUpdateText(ui->lbl_kCycleProfile,
                   m_gearHandler.settings().kCycleProfile);
  lambdaUpdateText(ui->lbl_keyboardGearUp,
                   m_gearHandler.settings().keyboardSeqGearUp);
  lambdaUpdateText(ui->lbl_keyboardGearDown,
                   m_gearHandler.settings().keyboardSeqGearDown);
  ...
  ui->cb_use_seq_after_clutch->setChecked(
      m_gearHandler.settings().useSequentialAfterClutch);
  ...
  lambdaUpdateText(ui->lbl_btn_switchMode, m_gearHandler.settings().switchMode,
                   false);
  lambdaUpdateText(ui->lbl_btn_cycleProfile,
                   m_gearHandler.settings().cycleProfile, false);
}
...
void MainWindow::UpdateConflicts() {
  ...
  if (m_gearHandler.settings().keyboardSeqGearUp ==
      m_gearHandler.settings().seqGearUp) {
    ::SetLabelHasConflict(
        ui->lbl_keyboardGearUp,
        tr("In game config: %2").arg(ui->pb_selectKey_SeqUp->text()));
    ::SetLabelHasConflict(
        ui->lbl_seqUp,
        tr("Keyboard inputs: %2").arg(ui->pb_selectKey_keyboardGearUp->text()));
  ...
  if (m_gearHandler.settings().keyboardSeqGearDown ==
      m_gearHandler.settings().seqGearDown) {
    ::SetLabelHasConflict(
        ui->lbl_keyboardGearDown,
        tr("In game config: %2").arg(ui->pb_selectKey_SeqDown->text()));
    ::SetLabelHasConflict(ui->lbl_seqDown,
                          tr("Keyboard inputs: %2")
                              .arg(ui->pb_selectKey_keyboardGearDown->text()));
    has_at_least_one_conflict = has_at_least_one_conflict || true;
  } else {
...
void MainWindow::onControllerButtonPressed(int button) {
  ...
  auto setStyleSheet{
      [&](auto *lbl, const auto &style) { lbl->setStyleSheet(style); }};
  ...
  if (button == m_gearHandler.settings().gearUp) {
    ...
    QTimer::singleShot(displayDelay, this, [&]() {
      setStyleSheet(ui->lbl_pad_LB, background_released);
    });
  } else if (button == m_gearHandler.settings().gearDown) {
    setStyleSheet(ui->lbl_pad_RB, background_pressed);
    m_gearHandler.gearDown();
    QTimer::singleShot(displayDelay, this, [&]() {
      setStyleSheet(ui->lbl_pad_RB, background_released);
    });
...
void MainWindow::on_cb_selectDevice_currentIndexChanged(int index) {
  ...
  if (index == -1 &&
      qsdl::getPluggedJoysticksCount() ==
          0) // no device selected and no available device connected
  {
    ...
    if (QFileInfo::exists(
            getProfileFilePath(deviceName))) // If the profile file exists
    {
      if (!loadProfile(deviceName)) // and we can't load the profile
                                    // corresponding to the device
      {
        throw std::runtime_error{
            __CURRENT_PLACE_std_ + " : Cannot load profile for <" +
    ...
    qInfo() << __PRETTY_FUNCTION__
            << "Connecting & registering new controller from mainwindow:"
            << ui->cb_selectDevice->currentData().toInt();
...
void MainWindow::on_pb_bgHUDColor_clicked() {
  ...
  auto color{QColorDialog::getColor(m_softSettings.bgHUDColor(), this,
                                    tr("Select background HUD color"))};
  if (!color.isValid())
    return;
...
void MainWindow::on_pb_changeBackground_clicked() {
  const auto kImage{QFileDialog::getOpenFileName(
      ...
      tr("Image Files %0;;All (*)").arg("(*.png *.jpg *.bmp *.jpeg)"))};
...
void MainWindow::on_action_checkUpdates_triggered() { m_updateHandler->show(); }

void MainWindow::on_actionOpen_logs_folder_triggered() {
  QFileInfo fInfo{
      QString::fromStdString(logHandler::GlobalLogInfo::i().progLogFilePath)};
